...
PairLJCutCoulPs::PairLJCutCoulPs(LAMMPS *lmp) : Pair(lmp)
{
...
PairLJCutCoulPs::~PairLJCutCoulPs() 
{
  if (copymode) return;
...
void PairLJCutCoulPs::compute(int eflag, int vflag)
{
        ...
        if (rsq < cut_coulsq) {
          if (!ncoultablebits || rsq <= tabinnersq) {
            ...
            for(int index=1; index<num_of_force_poly; index++){
               ...
               //printf("tabinnersq: %f, rsq: %f, r: %f, force_poly_appx: %f, coeff: %f\n", tabinnersq, rsq, r_scal, force_poly_appx, force_poly_coeff[index]);
            }
            ...
            // printf("tabinnersq: %f, rsq: %f, r: %f, table: %f\n", tabinnersq, rsq, r, table);
          ...
          if (rsq < cut_coulsq) { 
            if (!ncoultablebits || rsq <= tabinnersq){
              ...
              for(int index = 1; index<num_of_energy_poly; index++){
                ...
                //printf("tabinnersq: %f, rsq: %f, r: %f, energy_poly_appx: %f\n  coeff: %f\n", tabinnersq, rsq, r_scal, energy_poly_appx, energy_poly_coeff[i]);
              } 
              //printf("rsq: %f, r: %f, energy_poly_appx: %f\n", rsq, r_scal, energy_poly_appx); 
              //printf("%f   %f    %f\n", energy_poly_coeff[0], energy_poly_coeff[1], energy_poly_coeff[2]);
...
void PairLJCutCoulPs::compute_inner()
{
...
void PairLJCutCoulPs::compute_middle()
{
  int i,j,ii,jj,inum,jnum,itype,jtype;
  double qtmp,xtmp,ytmp,ztmp,delx,dely,delz,fpair;
  double rsq,r2inv,r6inv,forcecoul,forcelj,factor_coul,factor_lj;
...
void PairLJCutCoulPs::compute_outer(int eflag, int vflag)
{
...
void PairLJCutCoulPs::settings(int narg, char **arg)
{
...
void PairLJCutCoulPs::coeff(int narg, char **arg)
{
  if (narg < 4 || narg > 5)
...
void PairLJCutCoulPs::init_style()
{
  if (!atom->q_flag)
...
double PairLJCutCoulPs::init_one(int i, int j)
{
  if (setflag[i][j] == 0) {
...
void PairLJCutCoulPs::write_restart(FILE *fp)
{
...
void PairLJCutCoulPs::read_restart(FILE *fp)
{
...
void PairLJCutCoulPs::write_restart_settings(FILE *fp)
{
...
void PairLJCutCoulPs::read_restart_settings(FILE *fp)
{
  if (comm->me == 0) {
...
void PairLJCutCoulPs::write_data(FILE *fp)
{
...
void PairLJCutCoulPs::write_data_all(FILE *fp)
{
  for (int i = 1; i <= atom->ntypes; i++)
...
double PairLJCutCoulPs::single(int i, int j, int itype, int jtype,
                                 double rsq,
...
void *PairLJCutCoulPs::extract(const char *str, int &dim)
{
  dim = 0;
